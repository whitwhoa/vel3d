...
namespace vel
{
	...
	public:
		...
		virtual const std::vector<std::pair<std::string, VtxLayout>>& preload(const std::string& filePath) = 0;
		...
		virtual std::vector<std::unique_ptr<Mesh>> load(std::vector<std::pair<std::string, GeoPool*>>* loadables) = 0;
